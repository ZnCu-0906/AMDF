...
class SwinTransformerBlock(nn.Module):
    ...
    def flops(self):
        ...
        flops1 += nW * self.attn.flops(self.window_size * self.window_size)
        # mlp
        flops1 += 2 * H * W * self.dim * self.dim * self.mlp_ratio
        # norm2
        flops1 += self.dim * H * W
        return flops1
...
class SwinTransformer(nn.Module):
    ...
    def flops(self):
        flops1 = 0
        flops1 += self.patch_embed.flops()
        for i, layer in enumerate(self.layers):
            flops1 += layer.flops()
        flops1 += self.num_features * self.patches_resolution[0] * self.patches_resolution[1] // (2 ** self.num_layers)
        flops1 += self.num_features * self.num_classes
        return flops1
